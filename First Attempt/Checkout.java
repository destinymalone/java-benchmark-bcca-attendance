...
public class Checkout {
    ...
    public static void main(String[] args) {
        ArrayList<Attendance> checkouts = loadCheckouts();
        ArrayList<Attendance> checkoutTimes = checkoutTime(0, null, null);

        Attendance roll = editRoll();

        System.out.println("Sign in: ");
        while (true) {
            String choice = getStudentChoice();
            // String date = checkoutTime(0, choice, null);
            if (choice == null) {
                break;
            }
    ...
    private static ArrayList<Attendance> checkoutTime(long partialAbsences, ArrayList<Attendance> name,
            Boolean choice) {
        java.util.Date date = new java.util.Date();
        java.sql.Date dateTime = new java.sql.Date(date.getTime());
        System.out.println(dateTime);

        boolean isAfter = dateTime.getTime() > new Date(partialAbsences).getTime();

        if (choice == isAfter) {
            System.out.print(name + " checked out.");
        } else {
            System.out.print("Everyone stayed in class!");
        }
        return name;
    }

    private static void saveCheckouts(ArrayList<Attendance> checkouts) {
        try {
            FileOutputStream fileStream = new FileOutputStream("checkout.ser");
            ObjectOutputStream os = new ObjectOutputStream(fileStream);
            os.writeObject(checkouts);
            os.close();
        } catch (IOException ex) {
            System.out.println("Failed to save checkouts.");
        }
    }

    private static void saveCheckout(ArrayList<Attendance> checkoutTimes) {
        try {
            FileOutputStream fileStream = new FileOutputStream("checkout.ser");
            ObjectOutputStream os = new ObjectOutputStream(fileStream);
            os.writeObject(checkoutTimes);
            os.close();
        } catch (IOException ex) {
            System.out.println("Failed to save checkouts.");
        }
    ...
    private static Integer getTotalPresent() {
        while (true) {
            System.out.print("Checked Out[1]/Didn't Check Out[2] ");
            try {
    ...
    public static Attendance editRoll() {
    ...
    public static ArrayList<Attendance> loadCheckouts() {
        try {
            ...
            ArrayList<Attendance> checkout = (ArrayList<Attendance>) os.readObject();
            os.close();
            return checkout;
